...
public class FirebaseDatabaseConnectionHandler implements Application.ActivityLifecycleCallbacks {
    ...
    public void onActivityStarted(Activity activity) {
        count++;
        Util.Log("onActivityStarted: count=" + count);
        if (count > 0)
    ...
    public void onActivityStopped(Activity activity) {
        count--;
        Util.Log("onActivityStopped: count=" + count);
        if (count == 0) {

            Util.Log("onActivityStopped: going offline in a minute..");
            mHandler.postDelayed(new Runnable() {
                @Override
                public void run() {
                    // just make sure that in the defined seconds no other activity is brought to front
                    Util.Log("run: confirming if it is safe to go offline. Activity count: " + count);
                    if (count == 0) {
                        Util.Log("run: going offline...");
